...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $daily_calorie_goal = isset($_POST['daily_calorie_goal']) && $_POST['daily_calorie_goal'] !== '' ? intval($_POST['daily_calorie_goal']) : null;

    error_log("Received - Daily Calorie Goal: " . ($daily_calorie_goal ?? 'Not provided'));

    // Check if at least one field is provided
    if ($daily_calorie_goal === null) {
        echo json_encode(["message" => "No calorie goal provided to update."]);
        exit();
    }

    try {
        $stmt = $db->prepare("SELECT daily_calorie_goal FROM users WHERE user_id = :user_id");
        ...
        $daily_calorie_goal = $daily_calorie_goal ?? $currentData['daily_calorie_goal'];

        // Perform the update
        $stmt = $db->prepare("UPDATE users SET daily_calorie_goal = :daily_calorie_goal WHERE user_id = :user_id");
        $stmt->execute([
        ...
        if ($stmt->rowCount() > 0) {
            echo json_encode(["message" => "Calorie goal updated successfully!"]);
        } else {
