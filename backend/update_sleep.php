...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $daily_sleep_goal = isset($_POST['daily_sleep_goal']) && $_POST['daily_sleep_goal'] !== '' ? intval($_POST['daily_sleep_goal']) : null;

    error_log("Received - Daily Sleep Goal: " . ($daily_sleep_goal ?? 'Not provided'));

    // Check if at least one field is provided
    if ($daily_sleep_goal === null) {
        echo json_encode(["message" => "No step goal provided to update."]);
        exit();
    }

    try {
        $stmt = $db->prepare("SELECT daily_sleep_goal FROM users WHERE user_id = :user_id");
        ...
        $daily_sleep_goal = $daily_sleep_goal ?? $currentData['daily_sleep_goal'];

        // Perform the update
        $stmt = $db->prepare("UPDATE users SET daily_sleep_goal = :daily_sleep_goal WHERE user_id = :user_id");
        $stmt->execute([
            ":daily_sleep_goal" => $daily_sleep_goal,
            ":user_id" => $user_id
        ]);
        ...
        if ($stmt->rowCount() > 0) {
            echo json_encode(["message" => "Sleep Time goal updated successfully!"]);
        } else {
