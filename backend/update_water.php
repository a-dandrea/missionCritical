...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $daily_water_goal = isset($_POST['daily_water_goal']) && $_POST['daily_water_goal'] !== '' ? intval($_POST['daily_water_goal']) : null;

    error_log("Received - Daily Water Goal: " . ($daily_water_goal ?? 'Not provided'));
    ...
    try {
        $stmt = $db->prepare("SELECT daily_water_goal FROM users WHERE user_id = :user_id");
        ...
        $daily_water_goal = $daily_water_goal ?? $currentData['daily_water_goal'];

        // Perform the update
        $stmt = $db->prepare("UPDATE users SET daily_water_goal = :daily_water_goal WHERE user_id = :user_id");
        $stmt->execute([
        ...
        if ($stmt->rowCount() > 0) {
            echo json_encode(["message" => "Water intake goal updated successfully!"]);
        } else {
